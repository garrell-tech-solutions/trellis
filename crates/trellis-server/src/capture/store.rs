...
mod tests {
    ...
    async fn a_freshly_inserted_capture_carries_no_tag() {
        ...
        let id = insert(&pool, "buy milk", "web", 0).await.unwrap();
    ...
    async fn set_context_tag_leaves_other_captures_untouched() {
        ...
        let untouched = insert(&pool, "buy milk", "web", 1).await.unwrap();
    ...
    async fn canonical_tag_returns_the_earliest_spelling_when_several_exist() {
        ...
        // Bypasses resolve_tag's own canonicalization on purpose, to prove
        // the query itself -- not the caller -- picks the earliest row.
    ...
    async fn distinct_tags_lists_each_tag_once_earliest_use_first() {
        ...
        let first = insert(&pool, "buy screws", "web", 0).await.unwrap();
        set_context_tag(&pool, first, "@homedepot").await.unwrap();
        let second = insert(&pool, "pick up milk", "web", 1).await.unwrap();
        set_context_tag(&pool, second, "@supermarket")
            .await
            .unwrap();
        let third = insert(&pool, "return the drill", "web", 2).await.unwrap();
