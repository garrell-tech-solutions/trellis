# Trellis — Decisions Log: History

**The narrative half of `docs/decisions.md`.** One dated entry per slice or
settlement, in the order they happened: what the pipeline found, what it got
wrong, what an entry cost, and the arguments behind rows that ended up as one
line in a table.

**This file is append-only and nothing has been removed from it.** It was
split out of `docs/decisions.md` on 2026-08-26 because that file is read by
every agent at startup, from a worktree, offline — and half of what they were
reading was history rather than instruction.

## Which file to read

- **`docs/decisions.md`** — the settled `D-` / `T-` rows, the `R-` rejections,
  and how the IDs work. **That is the one to read at startup**, and the one
  `scripts/ci/decision_slugs.sh` checks citations against: a slug is *defined*
  by its table row there, never by a mention here.
- **This file** — why a row says what it says, and what was tried first.
  **Read it when a row surprises you**, when you are about to re-propose
  something, or when you need the argument rather than the conclusion.

**New entries go at the bottom.** A settled decision still gets its row in
`docs/decisions.md`; the entry here is the reasoning that produced it.
