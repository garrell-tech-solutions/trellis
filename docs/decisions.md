...
### 2026-08-25 — The owner's vision for v2, and the gate that made it survivable

**`D-the-browser-owns-the-database` records a direction, not a plan.** The owner
described it in one paragraph; what is settled is which of three shapes it takes
and nothing else. **It is deliberately unscheduled** — the Dogfood milestone is
eight days out with four slices queued, and starting this now costs the thing the
project is nine days from having.

**The best news in it is retrospective.** `T-core-no-tokio` and `T-module-boundary`
were argued for on testability grounds and enforced by a purity gate that has
looked, more than once, like ceremony. **That gate is why `scheduler-core`
compiles to WASM unchanged** — the rules, the interval algebra, the invariants,
`pool::group` and its new run rule all survive the deletion of the entire HTTP
tier. **A layering rule justified by one argument turned out to be load-bearing
for a completely different one**, which is the case for keeping such rules when
their stated reason looks thin at the time.

**And the bad news is symmetrical.** Six settled `T-` rows are about a request
that stops existing: `T-templates-take-view-models`, `T-forms-swap-one-fragment`,
`T-422-is-product-wide`, `T-nav-is-the-site-map`, `T-ephemeral-view-state-rides-the-request`
and, hardest of all, **the transport of all 24 acceptance features.** #126
recorded that *"the acceptance suite speaks only HTTP"* was a premise disguised
as a fact; under this direction it stops being either. **None of those rows is
wrong — they are all about a tier this direction removes**, which is a different
thing from being superseded, and they should be marked that way when the time
comes rather than deleted.

**One correction to advice this file's author gave an hour earlier.** I told the
owner that **#123** — no JSON read surface — *"stops being backlog cleanup and
becomes a prerequisite."* **That was said against a server-holds-the-data reading
and is wrong under this one.** If the browser owns the database, the server
serves a bundle and a sync endpoint; **there is no server-held data for a JSON
read surface to expose.** #123 keeps whatever value it had on its own terms and
gains nothing from this direction.

**The two risks worth naming before anyone is emotionally invested.** **OPFS is
evictable** — `navigator.storage.persist()` is a request, browsers evict under
pressure, and iOS has historically been the strictest. **A task app that silently
loses your commitments has the one failure it cannot have**, so this is a spike
before it is a plan. And **offline-first makes divergence the normal case**: two
devices editing the same database is not an edge, it is Tuesday. Last-writer-wins
on a whole SQLite file loses a day of captures without a word — the same
silently-unrepresentable failure `T-commitment-is-chosen-not-derived` refused.

**A smaller collision worth flagging early:** `stack.prompt` says *"Frontend:
Askama + HTMX, served by the backend. **No Node build step.**"* A browser-owned
database needs a client renderer, and only the Rust-to-WASM options (Leptos,
Dioxus, Yew) stay inside that constraint. **That is a real constraint on the
choice, not a detail to settle later**, and it should be decided knowingly rather
than discovered by whoever first reaches for a bundler.

**Finally, it resolves a gap opened the same day.** The product notifies nobody of
anything and never decided not to. A browser-owned database means **a service
