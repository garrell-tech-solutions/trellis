...
**The standard's third invariant is *opaque retrieval*, and it has a gate now (`#138`).** A capability that hands its whole table to another capability has broken it whether or not either one sorts: the caller now knows there is a list, and the day the query changes, the caller changes too. `scripts/ci/capability_front_doors.sh` fails the build when one capability's **production** code names another's `store` — `T-one-front-door-per-capability` reduced to something a script can check. It is scoped to `store` rather than to every submodule (`inbox::view::CaptureRow` is a shared view model, deliberately consumed by `capture::http`) and it exempts `#[cfg(test)]`, where every capability builds fixtures through `triage::store::insert_task` and always will. All three of its verdicts were proved able to fail before it was wired in, including its own self-check for the "tests are the file's last item" assumption it rests on (`T-a-check-must-be-seen-to-fail`).

**Worked example, and what it did not fix.** `triage::http` used to fetch every quota (`quota::existing_names`) and run `scheduler_core::quota::check_name` over the result itself. It now asks `quota::name_standing(pool, candidate)` and gets back `Free`, `Taken` or `Resembles`; `existing_names` is `pub(super)` and no longer leaves the capability. **The rule did not move into SQL and the Gap below is still open** — what moved is the boundary. `existing_names` still has no `WHERE`, and the day the exact tier can execute in the query, one function changes instead of two modules.
...
| `quota::check_name` runs over every quota fetched into memory; `store::existing_names` has no `WHERE` | the company set-operations standard | **new at #93, narrowed at #138** — the similar tier (Levenshtein, containment) cannot execute in SQLite at all; the exact tier could, but only against a stored normalized name, which needs a ruling on quota identity. **Still the specifier's**, and still open. What changed is the blast radius: the fetch and the comparison now both live inside `quota::name_standing`, behind the capability's front door, so `triage` no longer sees a list at all and the fix is one function rather than two modules |
...
| `scripts/ci/complexity_baseline.sh` pins **exact scores** for the Gherkin step dispatchers, so **every slice that adds or removes a step turns it red on arrival** | the `quality` CI job | **filed at #138, that instance closed by #152 — and red again at #140** with six baselined scores moved (`committed_screen` 18→19, `inbox_view` 11→17, `pool_screen` 28→31, `context_tags` 10→9, `triage` 16→15, `triage_from_page` 14→11), every one of them a dispatch chain gaining or losing an arm, none in a file the architect touched. **The #138 instance was different and is worth keeping separate**: two of its entries were not dispatch chains at all but straight-line fixture builders scoring on `?` and `map_err`, which is the case the threshold exists for, and it was fixed as such. **This one is pure accounting, and it recurs by construction.** The gate's rule — *"a dispatch chain gaining branches is a new step; a dispatch chain gaining *conditionals* is logic sneaking into the match"* — is the distinction worth gating, and an exact-score pin cannot draw it: it reports both identically, every slice, and the reviewer who has updated six numbers by hand is the one who stops reading the seventh. **The cleaner's** to update; **whether an exact pin is the right instrument is the owner's** |
