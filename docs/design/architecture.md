...
| `scripts/ci/complexity_baseline.sh` is red on arrival at `#138`: three functions over 8 and not in the baseline, three baselined scores moved | the `quality` CI job | **new at #138, and none of it is the architect's diff** — `disclosures.rs::dispatch` 10 and the three moves (`mod.rs::dispatch` 25→26, `quota_screen.rs::dispatch` 17→13, `triage.rs::dispatch` 18→16) are dispatch-chain accounting for steps this slice added and removed, which the baseline exists to absorb. **The other two are not dispatch chains**: `quota_migration.rs::dispatch_given_quota_task` (13) and `::given_pre_0016_database` (9) are straight-line fixture builders with no branch in them at all — every point is a `?` or a `map_err`, which `rust-code-analysis` counts. So the gate's own instruction splits: baseline the four, and *extract* rather than baseline the two. **The cleaner's**, whose lane complexity is |
