...
2. Open `http://localhost:8080`. Confirm there is a header with six links:
   **Inbox**, **Life areas**, **Free time**, **Capacity**, **Schedule**, **Stats**.
...
1. `GET /`, `GET /life-areas`, the free time page, the capacity page, the schedule page and `GET /stats`.
2. Extract the header region from each.
3. Compare the six, ignoring only the current-page marking.

### Expected Observable Outcomes
- All six carry a header offering exactly **Inbox**, **Life areas**,
  **Free time**, **Capacity**, **Schedule** and **Stats** — the same six, in the same order, with the
  same labels and the same targets.
- Apart from which entry is marked current, the six are **identical**. Any
...
- **Exactly six links.** `#59` added guardrails to the life areas page rather
  than a page of its own, so there is no guardrails link; `#60` added Free time and `#62` added
  Capacity, each page and link together. A link to a page that does not exist is worse
  than no link.

## Procedure — the current page is marked, and only it

1. `GET /` and read the header.
2. Repeat for `GET /life-areas`, the free time page, the capacity page, the schedule page and `GET /stats`.
...
- Each page marks **its own** entry as current: the inbox marks Inbox, the
  ...
  capacity page marks Capacity, the stats page marks Stats.
...
1. `GET /` and read the six links' targets out of the header.
...
- The link labelled **Inbox** reaches a page marking Inbox current; the same
  for Life areas, Free time, Capacity, Schedule and Stats.
...
3. `GET /`, `GET /life-areas`, the free time page, the capacity page, the schedule page and `GET /stats`, and read
