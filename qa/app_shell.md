...
2. Open `http://localhost:8080`. Confirm there is a header with five links:
   **Inbox**, **Life areas**, **Free time**, **Capacity**, **Stats**.
...
1. `GET /`, `GET /life-areas`, the free time page, the capacity page and `GET /stats`.
2. Extract the header region from each.
3. Compare the five, ignoring only the current-page marking.

### Expected Observable Outcomes
- All five carry a header offering exactly **Inbox**, **Life areas**,
  **Free time**, **Capacity** and **Stats** — the same five, in the same order, with the
  same labels and the same targets.
- Apart from which entry is marked current, the five are **identical**. Any
...
- **Exactly five links.** `#59` added guardrails to the life areas page rather
...
2. Repeat for `GET /life-areas`, the free time page, the capacity page and `GET /stats`.
...
1. `GET /` and read the five links' targets out of the header.
...
- The link labelled **Inbox** reaches a page marking Inbox current; the same
  for Life areas, Free time, Capacity and Stats.
...
3. `GET /`, `GET /life-areas`, the free time page, the capacity page and `GET /stats`, and read
